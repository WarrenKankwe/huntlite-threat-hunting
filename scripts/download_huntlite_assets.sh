...
# gdown is installed through gui/requirements.txt
if ! "$PYTHON_BIN" -c "import gdown" >/dev/null 2>&1; then
    echo "ERROR: gdown is not installed."
    echo "Activate the HUNT-LITE virtual environment and run:"
    echo "python -m pip install -r gui/requirements.txt"
    exit 1
fi
...
download_file () {
    ...
    if [[ "$OUTPUT" == *.csv ]]; then
        if head -c 512 "$TEMP" | grep -qiE '<!DOCTYPE html|<html'; then
            rm -f "$TEMP"
            echo "ERROR: Expected CSV but received HTML: $OUTPUT"
            return 1
        fi

        if ! "$PYTHON_BIN" - "$TEMP" <<'PY'
import csv
import sys
...
try:
    ...
    if len(header) < 2:
        raise ValueError("CSV header contains fewer than two columns")
...
PY
        then
            rm -f "$TEMP"
            echo "ERROR: CSV validation failed for $OUTPUT"
            return 1
        fi
    fi

    mv "$TEMP" "$OUTPUT"
    echo "Verified download: $OUTPUT"
}

download_raw_data () {
    download_file "$RAW_DATA_ID" \
        "data/raw/cybersecurity_threat_detection_logs.csv"
}

download_processed_data () {
    download_file "$PROCESSED_DATASET_ID" \
        "data/processed/huntlite_ml_dataset.csv"

    download_file "$TEST_SAMPLES_ID" \
        "data/processed/huntlite_streamlit_test_samples.csv"
}

download_project_assets () {
    download_file "$MODEL_ID" \
        "models/huntlite_threat_detection_model.keras"

    download_file "$PREPROCESSOR_ID" \
        "artifacts/preprocessor.joblib"

    download_file "$FEATURE_INFO_ID" \
        "artifacts/feature_info.json"

    download_file "$LABEL_MAPPING_ID" \
        "artifacts/label_mapping.json"

    download_file "$VERSION_INFO_ID" \
...
echo "Download and basic validation complete."
